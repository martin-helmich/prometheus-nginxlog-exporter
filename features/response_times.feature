Feature: Upstream response times are summarized
  ...
  Scenario: Single request is summarized
    ...
    When the following HTTP request is logged to "access.log"
      ...
      """
    Then the exporter should report value 10 for metric nginx_http_upstream_time_seconds{method="GET",quantile="0.5"}
  ...
  Scenario: .5 quantile is computed
    ...
    When the following HTTP request is logged to "access.log"
      ...
      """
    Then the exporter should report value 20 for metric nginx_http_upstream_time_seconds{method="GET",quantile="0.5"}

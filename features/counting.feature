Feature: Requests are counted
  ...
  Scenario: Single request is counted
    ...
    When the following HTTP request is logged to "access.log"
      ...
      """
    Then the exporter should report value 1 for metric nginx_http_response_count_total{method="GET"}
  ...
  Scenario: Multiple requests are counted
    ...
    When the following HTTP requests are logged to "access.log"
      ...
      172.17.0.1 - - [23/Jun/2016:16:04:20 +0000] "GET / HTTP/1.1" 200 612 "-" "curl/7.29.0" "-"
      """
    Then the exporter should report value 3 for metric nginx_http_response_count_total{method="GET"}
  ...
  Scenario: Requests are grouped by method
    ...
    When the following HTTP requests are logged to "access.log"
      ...
      """
    Then the exporter should report value 2 for metric nginx_http_response_count_total{method="GET"}
    And the exporter should report value 1 for metric nginx_http_response_count_total{method="POST"}
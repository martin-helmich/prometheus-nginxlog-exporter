...
default configuration of the official `nginx` image), then you can export logs using Docker's default logging driver if 
experimental features are enabled. To do this use a source configuration as follows:
...
The host's docker socket needs to be mounted to the container. For example, if using the docker run command:
